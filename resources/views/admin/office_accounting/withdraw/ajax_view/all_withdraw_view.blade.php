<table id="dataTableExample1" class="table table-bordered table-striped table-hover mb-2">
    ...
    <tr class="text-center">
        ...
        <td>{{ $withdraw->date }}</td>
        <td>{{ $withdraw->voucherHeader->name }}</td>
        <td>{{ $withdraw->bank->bank_name }}</td>
        <td>{{ $withdraw->account->holder_name }}</td>
        ...
        <td data-id="{{$loop->index}}">
            @if (json_decode($userPermits->office_accounts_module, true)['withdraw']['edit'])
                @if($withdraw->status==1)
                <a id="chenge_status_button" href="{{ route('admin.office.account.withdraw.change.status', $withdraw->id ) }}"
                ...
                |
            @endif
        <a href="#" data-id="{{ $withdraw->id }}" title="edit" class="edit_withdraw btn btn-sm btn-blue text-white">
        ...
        @if (json_decode($userPermits->office_accounts_module, true)['withdraw']['delete'])
            | <a id="delete_withdraw" href="{{ route('admin.office.account.withdraw.delete', $withdraw->id) }}"
